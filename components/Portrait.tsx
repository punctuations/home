...
import { VIEW, derivative } from "@/lib/system";

const X_CENTER = VIEW.xCenter;
const X_HALF = VIEW.xHalf;
const Y_CENTER = VIEW.yCenter;
const Y_HALF = VIEW.yHalf;
...
const SAMPLE = 240;
const DEGRADE_MS = 14;
const PROMOTE_MS = 3.5;

const FIELD_PIXELS = 3;
const FIELD_SPACING = 85;
const FIELD_LENGTH = 90;
const GRID_PX = 7;
const NEWTON_STEPS = 40;
const SEPARATRIX_STEPS = 900;
const SEPARATRIX_NUDGE = 0.004;
const CYCLE_BURN = 40000;
const CYCLE_POINTS = 2600;
...
function field(x: number, y: number, index: number) {
  derivative(x, y, flow);
  return flow[index];
}

function nextWait(mean: number) {
...
export default function Portrait() {
  ...
  useEffect(() => {
    ...
    function toScreenX(cx: number, x: number) {
      return cx + (x - X_CENTER) * xScale;
    }

    function toScreenY(cy: number, y: number) {
    ...
    function step(s: Float64Array, h: number) {
      const x = s[0];
      const y = s[1];
      const h2 = h * 0.5;
      const h6 = h / 6;
    ...
    function streamStep(s: Float64Array, sign: number) {
    ...
    function jacobian(x: number, y: number) {
      const hx = X_HALF * 1e-5;
      const hy = Y_HALF * 1e-5;
      return [
    ...
    function sampleGrid(cx: number, cy: number) {
    ...
    function contour(grid: Float64Array, cells: number[]) {
    ...
    function nullclineY(x: number, lowY: number, highY: number) {
      let lo = lowY;
      let hi = highY;
    ...
    function fastSlope(x: number, y: number) {
      const h = X_HALF * 1e-5;
    ...
    function findEquilibria(seeds: number[], cx: number, cy: number) {
      const found: number[] = [];
      ...
      for (const seed of seeds) {
        ...
        for (let n = 0; n < NEWTON_STEPS; n++) {
          ...
          if (
            Math.abs(stepX) * xScale < 1e-4 &&
            Math.abs(stepY) * yScale < 1e-4
          ) {
            solved = true;
            break;
          }
    ...
    function drawStructure(cx: number, cy: number) {
      if (!surfaceCtx) return;
        ...
        if (det < 0) {
          ...
          for (const outward of [1, -1]) {
            for (const direction of [1, -1]) {
              ...
              separatrix.moveTo(
                toScreenX(cx, trace[0]),
                toScreenY(cy, trace[1]),
              );
    ...
    function drawSurface(ratio: number) {
      if (!surfaceCtx) return;
