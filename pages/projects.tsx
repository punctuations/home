				...
				<section className="2xl:absolute xl:absolute left-5 top-24 2xl:mt-0 xl:mt-0 mt-12 2xl:grid xl:grid flex flex-col 2xl:space-y-0 xl:space-y-0 space-y-2 grid-cols-2 grid-flow-row gap-x-10 gap-y-8 h-5/12 2xl:w-5/6 xl:w-5/6 w-2/3 pb-5">
					{Project.map((project, i) => {
						return (
							<motion.div
								key={project.name}
								className={`relative overflow-hidden flex flex-col justify-center space-y-2 w-full 2xl:h-80 h-64 p-4 px-12 shadow-md hover:shadow-lg transition-all duration-500 dark:border-2 ${project.border} ${project.color} ${project.dark_color} ${project.text} ${project.dark_text} rounded-md`}
